...
- **New event types** (#811) – `IDENTITY_VERIFICATION_EXPIRED`, `IDENTITY_VERIFICATION_PENDING_PSC_ACTION`, `PSC_IDENTITY_VERIFICATION_PENDING`, `PSC_IDENTITY_VERIFICATION_VALIDATED`, `PSC_IDENTITY_VERIFICATION_FAILED`, `PSC_IDENTITY_VERIFICATION_EXPIRED`, and `PSC_IDENTITY_VERIFICATION_ABANDONED` on `EventType`.
...
### Added
- **`Report.DateRangeBy`** – New property on `Report` to align with the API. Use this instead of the deprecated `DateFilterBy`.

### Deprecated
- **`Report.DateFilterBy`** – Deprecated in favor of `DateRangeBy`. The property is kept for backward compatibility and will be removed in a future version.
...
- Support for the `Licensor` property on [GET View a Client](https://docs.mangopay.com/api-reference/client/view-client) (#749 )
...
New `ReportTypes` for Echo (#728, [API release note](https://docs.mangopay.com/release-notes/api/2025-08-06))):
- `ECHO_INTENT`
- `ECHO_INTENT_ACTION`
- `ECHO_SETTLEMENT`
- `ECHO_SPLIT`
